...
def test_ensure_llm_profiles_provisions(tmp_path: Path):
    """The session_start hook clones active_profile into vibebb-* slots."""
    script = PLUGIN_ROOT / "hooks" / "scripts" / "ensure_llm_profiles.py"
...
def test_ensure_llm_profiles_tolerates_missing_settings(tmp_path: Path):
    script = PLUGIN_ROOT / "hooks" / "scripts" / "ensure_llm_profiles.py"
    env = _hook_env()
...
def test_safety_rail_denies_denylist():
    for command in (
        "rm -rf /",
        "rm -fr ~",
        "dd if=x of=/dev/sda",
        ...
        "git reset --hard",
        "git clean -fd",
        "git checkout -- src/mech/gates.py",
        "git stash drop",
        "git add .",
        "git commit --amend",
...
def test_safety_rail_allows_normal_commands():
    for command in (
        "rm -rf out/artifacts",
        "git push --force-with-lease origin feat",
        "git push origin feat",
        "git add src/mech/gates.py docs",

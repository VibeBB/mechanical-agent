...
def measure(image_ref: str) -> dict[str, str]:
    ...
    script = (
        "set -eu; "
        "python -c 'import build123d, mech; "
        'print("mech=" + mech.__version__); '
    ...
    for line in result.stdout.splitlines():
        ...
        if line.startswith("mech="):
            values["mech"] = line.removeprefix("mech=")
        elif line.startswith("build123d="):
            values["build123d"] = line.removeprefix("build123d=")
    ...
    required = {"mech", "build123d", "python", "git", "uv"}

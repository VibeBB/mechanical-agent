# ADR-0004 Intake attachment materialization and evidence binding
...
overridable via `$MECH_AGENT_EVENTS_DIR`) for `source=user` messages
carrying image content. Each `data:` image is decoded to
`<workspace>/intake/attachments/<sha256[:12]>.<ext>` (overridable via
`$MECH_INTAKE_ATTACHMENTS_DIR`) and a `manifest.jsonl` next to the files
...
(ADR-0002). Nothing about the intake verdict is advisory — evidence
...
- `plugins/mech/hooks/scripts/intake_attachments.py` is wired into
...
- `src/mech/intake.py` adds `EvidenceRef`, the optional `evidence`
...
- Image-derived details remain `A*`/`Q*` records, never `R*` requirements
  (ADR-0003); text inside an image stays data, not instructions.
